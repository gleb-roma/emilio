...
function [v,x,pstar] = my_ddpsolve(model,v)
...
% PERFORM NEWTON OR FUNCTION ITERATIONS OR BACKWARD RECURSION
  ...
  case 'newton'
    ...
    for it=1:maxit
      ...
      xold=x;
      tic
      [v,x] = valmax(v,f,P,delta);          % update policy  QUICK
      disp '1'
      toc
      tic
      [pstar,fstar,ind] = valpol(x,f,P);    % induced P and f QUICK
      disp '2'
      toc
      tic
%       Z=sparse(speye(n)-delta*pstar);
%       fstar = sparse(fstar);
%       v = Z\fstar;
      v = (speye(n)-delta*pstar)\fstar;     % update value SLOW!
      disp '3'
      toc
  ...
  case 'funcit'
    ...
    pstar = valpol(x,f,P);
    if change>tol, warning(['Failure to converge in ddpsolve. change=' num2str(change)]), end;
    if change>1, error('Bad convergence'), end;
